...
const Cards = ({ item }) => {
  ...
  useEffect(() => {
    const getPdf = async () => {
      try {
        const result = await axios.get("https://book-store-lcz0.onrender.com/get-files");
  ...
  const addBookOnDb = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://book-store-lcz0.onrender.com/purchase/buy",
  ...
  const createOrder = async () => {
    try {
      ...
      const response = await axios.post("https://book-store-lcz0.onrender.com/create-order", {
  ...
  return (
   <div className='mt-4'>
    <div className="card bg-base-100 w-90 m-5 sm:m-4 shadow-xl hover:scale-105 duration-200 bg-white text-black dark:bg-slate-900 dark:text-white dark:border">
      <figure>
